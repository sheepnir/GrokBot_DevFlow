...
| 17 | Create separate environments, at least `test` and `production`, with their own credentials and their own cloud roles. Protect `production` (C7): `shpdev-cto` and the founder as required reviewers, "Prevent self-review" on, deployment branches limited to `main`, and a production cloud role whose OIDC trust accepts only this repository's `production` environment. Because self-review is prevented, whoever starts a production deployment can't approve it, so every production deployment needs both the CTO and the founder. Add a `production-rollback` environment whose only job restores the recorded previous known-good artifact and compatible configuration, with the same two reviewers and self-review allowed, so either of them can roll back alone. Give this environment a separate OIDC trust and a rollback-only workflow that accepts no arbitrary artifact or shell command; serialize it with normal deployment and check migration compatibility before restoring. | Cloud Engineer, with the founder for the protection settings | Nothing in `test` can reach `production`, a deployment to `production` waits for the other person's approval, and a rollback runs with one |
...
| 21 | Write the deployment and rollback procedure in `/docs/architect/operations.md`. The pipeline deploys the immutable CI artifact and records the environment, artifact digest, configuration/flag revision, migration state, and operation ID. It skips only a full desired state already confirmed live and serializes all deploy/rollback operations per environment. Before deployment it snapshots the previous verified healthy artifact and configuration as the rollback target; the first launch has no rollback target. Migrations record completion and recovery behavior for partial failure. Rollback checks data/schema compatibility and restores the recorded target, never a dynamically resolved latest tag. | Cloud Engineer | The rollback and retry tests in [the rollout checklist](rollout/activation-checklist.md) pass with evidence, including a failed deployment, changed flags with unchanged code, and incompatible migration |
...
1. Product feature work starts only after every checklist item is done and recorded. Bootstrap issues that implement this checklist are the explicit exception: the founder or CTO names the setup owner and scope, work stays on isolated branches, independent review is required, and no production deployment is allowed before deployment controls are tested. This avoids requiring CI and deployment automation before an issue can be claimed to build them. Test incomplete controls only with synthetic data and non-production targets.
